...
export default function SiteInquiryForm({ siteId, primaryColor }) {
  ...
  const submit = async (event) => {
    ...
    if (response.ok) {
      ...
      setState({ status: "error", message: "Unable to send your inquiry. Please try again." });
    }
  };
  ...
  return (
    <form onSubmit={submit} className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <div className="sm:col-span-2">
        {siteKey ? <Turnstile sitekey={siteKey} onVerify={setTurnstileToken} onExpire={() => setTurnstileToken("")} /> : <p className="text-sm text-red-700">Inquiry verification is unavailable.</p>}
      </div>
